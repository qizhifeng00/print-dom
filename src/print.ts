...
const IFRAME_STYLE =
  'position: absolute; top: 0; left: 0; width: 0; height: 0; border: none;';
...
export default class Print {
  ...
  private getStyle() {
    ...
    const styleList = Array.prototype.slice
      .call(styles)
      .map((s: Element) => s.outerHTML);
  ...
  private getNoPrintStyle() {
    ...
    if (isArray(noPrint) && (noPrint as string[]).length) {
      return `<style type="text/css">${noPrint!.join(
        ', '
      )} { display: none; }</style>`;
    } else {
      return '';
  ...
  private writeIframe() {
    const existedIframe = document.querySelector(
      'iframe#print-js-iframe'
    ...
    const doc =
      iframe.contentDocument || (iframe.contentWindow as Window).document;
    doc.open();
    doc.write(content);
    doc.close();
  }
